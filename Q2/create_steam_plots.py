...
DATASET_CSV = PROJECT_DIR / "final_ml_dataset_with_completion.csv"
...
def read_data():
    if not DATASET_CSV.exists():
        raise FileNotFoundError(f"Processed dataset not found: {DATASET_CSV}")

    data = pd.read_csv(DATASET_CSV, low_memory=False)
...
def plot_recommendation_rate(data):
    ...
    ax.set_xticks(x, [f"{label}\nn={count:,}" for label, count in zip(labels, summary["count"])], rotation=35, ha="right")
    ...
    ax.set_xlabel("Achievements completed by the reviewer", fontsize=15, labelpad=10)
    ax.set_ylabel("Reviews recommending the game", fontsize=15, labelpad=10)
...
def plot_heatmap(data, recommended, first_words, second_words, first_name, second_name, title, filename):
    ...
    ax.set_xlabel("Achievement completion group", fontsize=16, labelpad=10)
    ax.set_ylabel("Playtime when the review was written", fontsize=16, labelpad=12)
